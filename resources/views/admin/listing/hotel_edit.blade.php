...
<div class="ol-card mt-3">
    <div class="ol-card-body p-3">
        ...
        <form action="{{route('admin.listing.update',['type'=>'hotel', 'id'=>$listing->id])}}" id="form-action" method="post" enctype="multipart/form-data">
            @csrf
            <div class="tab-content pt-3" id="myTabContent">
                ...
                <div class="tab-pane fade show {{($tab == 'feature')?'active':''}}" id="feature" role="tabpanel" aria-labelledby="feature-tab">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        ...
                        <a href="javascript:void(0);" onclick="modal('modal-md', '{{route('admin.amenities.add',['type'=>'hotel','item'=>'feature','page'=>'listing','listing_id'=>$listing->id])}}', '{{get_phrase('Add New Service')}}')" class="btn btn-primary fs-14px"> {{get_phrase('Add Feature')}} </a>
                    </div>
                    @php
                ...
                <div class="tab-pane fade show {{($tab == 'room')?'active':''}}" id="room" role="tabpanel" aria-labelledby="room-tab">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        ...
                        <a href="javascript:void()" onclick="modal('modal-xl', '{{route('admin.add.listing.room',['id'=>$listing->id,'room_id'=>0, 'page'=>'add'])}}', '{{get_phrase('Add New Room')}}')" class="btn btn-primary fs-14px"> {{get_phrase('Add Room')}} </a>
                    </div>
                    @php
                        ...
                        <div class="col-sm-4"> 
                            ...
                            <label class="form-check-label w-100" onclick="room_select('{{$key}}')" for="flckDefault{{$key}}">
                                <div class="card mb-3 room-checkbox">
                                    <div class="row g-0">
                                        <div class="col-md-4">
                                            <img class="object-fit" src="{{ get_image('public/storage/room-images/' . (json_decode($room->image)[0] ?? 'default.jpg')) }}" class="img-fluid rounded-start" alt="Room Image">
                                        </div>
                                        <div class="col-md-8 room-body">
                                            <div class="card-body py-0 px-2 h-100 position-relative">
                                                <p class="card-title line-3"> {{$room->title}} </p>
                                                <p class="card-text fs-12px"> {{currency($room->price)}} </p>
                                                <div class="text-end position-absulate">
                                                    <a href="javascript:void(0);" onclick="modal('modal-xl', '{{route('admin.add.listing.room',['id'=>$listing->id,'room_id'=>$room->id, 'page'=>'edit'])}}', '{{get_phrase('Add New Room')}}')" class="p-1"> <i class="fas fa-edit"></i> </a>
                                                    <a href="javascript:void(0);" onclick="delete_modal('{{route('admin.delete.listing.room',['id'=>$room->id,'listing_id'=>$listing->id])}}')" class="p-1"> <i class="fas fa-trash-alt"></i> </a>
                                                </div>
                                            </div>
