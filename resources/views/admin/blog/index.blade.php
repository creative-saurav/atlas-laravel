    ...
    <div class="ol-card-body p-3">
        ...
        <table id="datatable" class="table" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    <td> {{$num++}} </td>
                    <td> <img class="rounded" src="{{get_image('public/storage/blog-images/'.$blog->image)}}" width="50px" height="50px"> </td>
                    <td> {{$blog->title}} </td>
                    <td> {{App\Models\Blog_category::where('id', $blog->category)->first()->name}} </td> 
                    <td> {{($blog->status == 1)?get_phrase('Active'):get_phrase('Pendding')}} </td>
                    <td> 
                        <div class="dropdown ol-icon-dropdown">
                            ...
                            <ul class="dropdown-menu">
                              ...
                              <li><a class="dropdown-item fs-14px" href="{{route('admin.blog.status',['id'=>$blog->id, 'status'=>$blog->status])}}"> {{get_phrase('Chagne Status')}} </a></li>
