...
@php
    $country_id = json_decode($user->address)->country;
    $city_id = json_decode($user->address)->city;
@endphp
        ...
        <form action="{{route('admin.update.user',['type'=>$type, 'id'=>$user->id])}}" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-sm-6 mb-3">
                    <label for="name" class="form-label ol-form-label"> {{get_phrase('User Full Nmae *')}} </label>
                ...
                <div class="col-sm-6 mb-3">
                    ...
                    <input type="email" class="form-control ol-form-control" name="email" id="email" placeholder="{{get_phrase('Enter email')}}" value="{{$user->email}}" required>
                </div>
                <div class="col-sm-6 mb-3">
                ...
                <div class="col-sm-6 mb-3">
                    ...
                    <select name="city" id="city" class="form-control ol-select2 ol-form-control" data-minimum-results-for-search="Infinity">
                        <option value="{{$city_id}}"> {{App\Models\City::where('id', $city_id)->first()->name}} </option>
                    </select>
                </div>
                <div class="col-sm-6 mb-3">
            ...
            <button type="submit" class="btn btn-primary fs-14px px-4"> {{get_phrase('Update')}} </button>
        </form>
    </div>
</div>
